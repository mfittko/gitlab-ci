module Charts
  ...
  class BuildTime < Chart
    def collect
      builds = project.builds.where('builds.finished_at is NOT NULL AND builds.started_at is NOT NULL AND builds.status = "success" AND builds.tag is NOT NULL AND builds.tag != "" AND TIMESTAMPDIFF(MINUTE, `builds`.`started_at`, `builds`.`finished_at`) > 42').last(Settings.charts['builds_time_span'])
      builds.each do |build|
        @labels << "#{build.finished_at.strftime('%d.%m.%Y')} - #{build.tag}"
